<template>
	<section class="loginContainer">
              ...
              <section>
                ...
                <section class="login_verification">
                  ...
                  <div class="switch_button" :class="{on:showPwd,off:!showPwd}" @click="showPwd=!showPwd">
                    <div class="switch_circle" :class="{switch_open:showPwd}"></div>
      ...
      <AlertTip :alertText="alertText" @closeTip="closeTip" v-if="alertShow"/>
    </section>
</template>
<script>
  ...
  methods:{
    async getCode(){
      if(!this.computeTime){
        ...
        const message = await reqSendCode(this.phone)
        if(message.code === 1){
          this.showAlert(message.msg)
            if(this.computeTime){
              this.computeTime = 0
              clearInterval(this.daoji)
              this.daoji = undefined
            }
        }
      }
    },
    ...
    async login(){
      ...
      if(result.code === 0){
        this.$store.dispatch('recodeUserInfo',result.data)
